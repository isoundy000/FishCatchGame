function iframe4Url(cfg) {
	...
	this.createIframeBox = function(url) {
		var egret_main = document.getElementById('egret_main');
		var clientWidth = egret_main.clientWidth;
		var clientHeight = egret_main.clientHeight;
		var wrapper = document.getElementById('wrapper');
		var body = document.body;
		var scale = Math.min(body.clientWidth, body.clientHeight) / 720;
		var canvas = document.getElementsByTagName('canvas')[0];
		var left = canvas.style.left.substring(0, canvas.style.left.length - 2);
		var top = canvas.style.top.substring(0, canvas.style.top.length - 2);
		...
		oDiv.style.cssText = "width:"+ (200*scale) +"px; height: "+(200*scale)+"px;position:fixed;z-index:" + this.iframeBoxzIndex + ";overflow:hidden;left:"+ left + (200*scale)+"px;top:"+ top + (200*scale)+"px";
		...
		oScript.style.cssText = "background-color:#fff;width:200px; height:200px;border:0";
		oScript.src = url;
	...
	this.open = function(url, onclose, w,h,x,y) {
		this.createIframeBox(url);
		this.createBack(onclose);
	};
	return this;
}
window.iframe4Url = new iframe4Url();